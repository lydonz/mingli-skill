...
当前稳定版本为 **v2.0.1**。`v2.0.0` 是一次不保留长期兼容性的重大升级，`v2.0.1` 修复了规则冲突、海外时区、安装后紫微依赖和候选命盘稳定性等问题；完整变化见 [CHANGELOG.md](CHANGELOG.md)。
...
从 `v2.0.0` 升级到 `v2.0.1` 后：

- 废弃旧旺衰字段不再制造普通命盘冲突，事业和财富分析不会被无条件降级。
- 未请求的流年、知识、解释和选择题组件统一返回 `skipped`。
- 海外用户的立春、农历新年和流月节气边界会转换到当地时区。
- 出生时间误差候选盘会同时记录八字与紫微的变化。
- `setup.sh`/`setup.bat` 会在 Skill 目标目录安装 `iztro`。
- 标准时间模式可只提供 IANA 时区；非法性别会被明确拒绝。
