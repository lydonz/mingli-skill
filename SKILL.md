...
`birth_time.effective_time` 用于日柱和时柱；`birth_time.calendar_time` 将出生民用时刻统一到节气后端时标，用于年柱和月柱的节气边界。不得将海外本地时间直接与中国节气时钟比较。
...
具体事件必须传 `analysis_period`。流年按 `year_boundary`（默认立春）切分，流月以十二个节气边界切分。`lunar_new_year` 必须同时影响本命年柱、大运、流年和流月。仅从问题文本解析年份是降级模式，必须保留其质量提示。

所有个人分析入口应先取得一份 `ComputedChart`，再传给八字、紫微、流年和解释模块。时间误差不跨边界时可使用稳定盘；跨边界时必须列出 `birth_time.chart_stability.candidate_charts` 中的全部候选盘及变化字段。
